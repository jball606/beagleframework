	...
	<? 
		...
		<? 
			...
			foreach($result['headers'] as $k => $i)
			{
				...
				if(strpos($i,$result['editaccess']['field']) === false && !isset($result['hiddencols'][$field]) && !isset($result['hiddencols'][$k]))
				{
					?>
					...
					?>
						<?
							...
							{ 
								...
								{ 
									if($order[$k] == 1)
									{ ?><div style="float:right" border="0" class="up_arrow"></div> <? }
									elseif($order[$k] == 2)
									{ ?><div style="float:right" class="down_arrow"></div><? } 
								}
							}
