...
class excel
{
	...
	public function csvweboutput($array,$filename)
	{

		header('Pragma: public');
		header("Expires: Mon, 25 Jul 1997 05:00:00 GMT");// Date in the past 
	...
	public function csvweboutputFile($tmpfilename,$filename)
	{
		header('Pragma: public');
		header("Expires: Mon, 25 Jul 1997 05:00:00 GMT");// Date in the past 
		header('Last-Modified: '.gmdate('D, d M Y H:i:s') . ' GMT');
		...
		header('Content-Disposition: attachment; filename="'.basename($filename).'.csv"');
		$x = 0;
		$h = fopen($tmpfilename,"r");
		while(!feof($h))
		{
			echo $this->csvline(unserialize(fgets($h)));	
		
		}
		fclose($h);
	}

	private function csvline($array)
	{
		$line = "";
		if(is_array($array))
		{
			foreach($array as $i)
			{
				$line .= '"'.$this->killchar($i).'",';
			}
		
			return substr($line,0,strlen($line)-1)."\r";
		}

	}
	
	public function weboutput($array,$filename)
	{
		$wb = new Spreadsheet_Excel_Writer();
		$wb->send($filename.".xls");
		$ws = & $wb->addWorksheet("Worksheet 1");
	...
	public function open($file,$rw="r",$encoding="CP1251",$outencode="iconv")
	{
		if(substr(strtolower($rw),0,1) == "r")
		{
			...
			$this->reader->setUTFEncoder($outencode);
			...
			**/
			...
			if 'type' == "unknown" - use 'raw' value, becausecell contain value with format '0.00';
		...
		if(substr(strtolower($rw),0,1) == "w")
		{
			$this->writer = fopen($file,"w");
		}
	}

	public function fetch_array($sheet=0,$numrow="")
	{
		if(!isset($this->whilecount[$sheet]))
		{ 
			$this->whilecount[$sheet] = 1; 
		}
		
		if($numrow == "")
		{
			$numrow = $this->reader->sheets[$sheet]['numCols']; 
		}
		
		error_reporting(E_ALL ^ E_NOTICE);
		
		if($this->reader->sheets[$sheet]['numRows']>=$this->whilecount[$sheet] && $this->whilecount[$sheet]<65000)
		{ 
			for ($j = 1; $j <= $numrow; $j++) 
			{
				$row[$j-1] = $this->reader->sheets[$sheet]['cells'][$this->whilecount[$sheet]][$j]; 
			}
		
			$this->whilecount[$sheet]++;
			return $row;
		}
		else
		{
			unset($this->whilecount[$sheet]);
			return false;
		}
	} 
	...
	public function DBExcel($result,&$db)
	{
		$line = "";
		$data = "";
	...
	private function killchar($junk)
	{
		$junk = trim($junk);
		$junk = str_replace('"', '"', $junk);
		$junk = str_replace("\t"," ",$junk);
		$junk = str_replace(chr(9)," ",$junk);
		//$junk = str_replace(" "," ",$junk);
		$junk = str_replace("\n",chr(13),$junk);
	...
	public function writeline($line,$print="N")
	{
		$data = "";
		if(is_array($line))
		{
			$nl = $this->CombineLine($this->CleanData($line)); 
		}
		else
		{
			$nl = trim($line); 
	...
	private function CombineLine($var)
	{
		$line = '';
		$key = array_keys($var);
...
* @packageSpreadsheet_Excel_Reader
* @author Vadim Tkachenko <vt@phpapache.com>
* @copyright1997-2005 The PHP Group
* @licensehttp://www.php.net/license/3_0.txtPHP License 3.0
* @versionRelease: @package_version@
* @link http://pear.php.net/package/PackageName
* @seeOLE, Spreadsheet_Excel_Writer
*/
